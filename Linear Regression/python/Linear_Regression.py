...
for k in range(n):
    learning_rate = 0.1 # (Ideal but can be triggered)
    theta = update_theta_with_rg(theta, learning_rate, lmb)
    cost_rg.append(cost_function_with_rg(theta, learning_rate, lmb))

print(f'Gradient Descent method with regularization: {theta}') # Parameter values



# Visualizing the cost function
plt.plot(np.arange(n), cost_rg)
